...
export async function getNowPlayingMovies(language: string = 'en-US', region: string = 'IN'): Promise<Movie[]> {
  if (!TMDB_API_KEY) return [];
  const url = `/api/tmdb/3/movie/now_playing?api_key=${TMDB_API_KEY}&language=${language}&region=${region}&page=1`;
  const response = await fetch(url);
  const data = await response.json();
  return data.results;
}
