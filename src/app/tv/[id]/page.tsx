            ...
            <div className="space-y-6 md:space-y-10 animate-in fade-in slide-in-from-bottom-10 duration-1000">
                <div className="flex flex-wrap items-center gap-3">
                    <Badge className="bg-primary text-white font-black uppercase text-[10px] px-4 py-1.5 rounded-sm shadow-2xl shadow-primary/20 flex items-center gap-2">
                        <ShieldCheck className="size-3" /> Original Series
                    </Badge>
                    <div className="flex items-center gap-2 text-yellow-400 font-black text-xs md:text-sm bg-black/60 backdrop-blur-3xl px-4 py-1.5 rounded-full border border-white/10 shadow-2xl">
                        ...
                        {show.vote_average.toFixed(1)} <span className="text-muted-foreground font-medium text-[10px] ml-1">Series Grade</span>
                    </div>
                    <div className="flex items-center gap-2 text-blue-400 font-black text-xs md:text-sm bg-black/60 backdrop-blur-3xl px-4 py-1.5 rounded-full border border-white/10 shadow-2xl">
                        ...
                        {Math.round(show.popularity)} <span className="text-muted-foreground font-medium text-[10px] ml-1">Metric</span>
                    </div>
          ...
          <div className="w-full lg:w-[450px] flex-shrink-0 space-y-12">
            ...
            <div className="grid grid-cols-1 gap-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                    <Button onClick={handlePlayTrailer} variant="outline" className="rounded-2xl h-16 md:h-20 border-white/10 glass-panel text-xs md:text-sm font-black uppercase tracking-widest transition-all hover:scale-105 active:scale-95" disabled={!trailerAvailable}>
                        <PlayCircle className="mr-2 size-5" /> Trailer
                    </Button>
                    <Button onClick={handleSaveToggle} variant={isSaved ? "secondary" : "outline"} className="rounded-2xl h-16 md:h-20 border-white/10 glass-panel text-xs md:text-sm font-black uppercase tracking-widest transition-all hover:scale-105 active:scale-95" disabled={isSavedShowLoading}>
                    ...
                    <div className="flex flex-wrap items-center gap-4">
                        <div className="flex items-center gap-8 bg-secondary/40 px-10 py-5 rounded-[2.5rem] border border-white/5 backdrop-blur-3xl shadow-2xl">
                            <div className="flex items-center gap-3">
                                ...
                                <span className="font-black uppercase tracking-widest text-xs">{show.first_air_date ? new Date(show.first_air_date).getFullYear() : 'TBA'} Start</span>
                            </div>
                            <Separator orientation="vertical" className="h-6 bg-white/10" />
                            <div className="flex items-center gap-3">
                                <LayoutGrid className="size-5 text-primary" />
                                <span className="font-black uppercase tracking-widest text-xs">{show.number_of_seasons} Full Cycles</span>
                            </div>
                        </div>
                ...
                <section className="space-y-12">
                    <div className="flex items-center justify-between border-b border-white/5 pb-8">
                        <div className="flex items-center gap-4">
                            ...
                            <div className="space-y-1">
                                <h2 className="font-headline text-3xl md:text-5xl font-black tracking-tighter uppercase mb-0">Primary <span className="text-primary">Personnel</span></h2>
                                <p className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.4em] opacity-60">The core identity units participating in this cycle.</p>
                            </div>
                        </div>
                        <Badge variant="outline" className="hidden sm:flex rounded-full px-6 py-2 border-white/10 text-muted-foreground text-[10px] font-black uppercase tracking-[0.2em]">Archived Profiles</Badge>
                    </div>
                        ...
                        {show.credits.cast.slice(0, 9).map(person => (
                            <Link href={`/person/${person.id}`} key={person.credit_id} className="flex items-center gap-5 p-5 glass-panel rounded-[2rem] hover:bg-white/10 transition-all border border-white/5 group shadow-xl">
                                ...
                                <div className="overflow-hidden space-y-1">
                                    ...
                                    <p className="text-[9px] text-muted-foreground truncate uppercase font-bold tracking-widest mt-1.5">{person.character}</p>
                                </div>
                            </Link>
                        ))}
                    </div>
                ...
                <section className="space-y-12">
                    <div className="flex items-center justify-between border-b border-white/5 pb-8">
                        <div className="flex items-center gap-4">
                            ...
                            <div className="space-y-1">
                                <h2 className="font-headline text-3xl md:text-5xl font-black tracking-tighter uppercase mb-0">Cycle <span className="text-primary">Transmissions</span></h2>
                                <p className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.4em] opacity-60">Full sequence indexing across all available seasons.</p>
                            </div>
                        </div>
                        <Badge variant="outline" className="hidden sm:flex rounded-full px-6 py-2 border-white/10 text-muted-foreground text-[10px] font-black uppercase tracking-[0.2em]">
                            {show.number_of_episodes} Total Sequences
                        </Badge>
                    </div>
                    
                    <div className="grid grid-cols-1 gap-8">
                        {show.seasons.filter(s => s.season_number > 0).map(season => (
                            <div key={season.id} className="group glass-panel rounded-[3rem] overflow-hidden border border-white/5 hover:border-primary/20 transition-all duration-700 shadow-2xl relative">
                                <div className="flex flex-col md:flex-row gap-10 p-10 relative z-10">
                                    <div className="w-full md:w-56 aspect-[2/3] relative flex-shrink-0 rounded-[2rem] overflow-hidden border border-white/10 shadow-[0_30px_60px_rgba(0,0,0,0.8)] group/poster">
                                        {season.poster_path ? (
                                            ...
                                            <div className="w-full h-full bg-secondary flex items-center justify-center">
                                                <Tv className="size-16 text-white/10" />
                                            </div>
                                        )}
                                        <div className="absolute inset-0 bg-black/40 opacity-0 group-hover/poster:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-sm">
                                            <Button size="icon" className="h-16 w-16 rounded-full bg-primary shadow-2xl" onClick={() => handlePlayNow(season.season_number, 1)}>
                                                <Play className="size-8 fill-current text-white ml-1" />
                                            </Button>
                                        </div>
                                    </div>
                                    <div className="flex-1 space-y-8">
                                        <div className="flex items-start justify-between">
                                            <div className="space-y-2">
                                                <h3 className="font-black text-2xl md:text-5xl group-hover:text-primary transition-colors leading-none text-white tracking-tighter uppercase">{season.name}</h3>
                                                <div className="flex items-center gap-4">
                                                    <span className="text-[10px] font-black text-primary uppercase tracking-[0.3em]">{season.air_date ? new Date(season.air_date).getFullYear() : 'TBA'} Sequence</span>
                                                    <Separator orientation="vertical" className="h-4 bg-white/10" />
                                                    <span className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.3em]">{season.episode_count} Archived Transmissions</span>
                                                </div>
                                            </div>
                                            <Button variant="outline" className="hidden md:flex rounded-full px-10 h-14 glass-panel hover:bg-primary transition-all border-white/10 font-black uppercase tracking-widest text-[10px]" onClick={() => handlePlayNow(season.season_number, 1)}>
                                                <Play className="mr-3 size-4 fill-current" />
                                                Initialize Cycle
                                            </Button>
                                        </div>
                                        <p className="text-lg md:text-2xl text-muted-foreground/90 leading-relaxed font-medium line-clamp-4">
