...
import { Loader2, Play, Star, Bookmark, Calendar, Clock, ChevronLeft, Share2, TrendingUp, Users, Award, Clapperboard, Smartphone, Zap, Copy, ShieldCheck } from 'lucide-react';
...
export default function MovieDetailsPage(props: { params: Promise<{ id: string }> }) {
  ...
  const handleExternalPlayer = (player: 'vlc' | 'mx') => {
    if (!movie) return;
    const streamUrl = `https://vidsrc.wtf/api/1/movie/?id=${movie.id}&color=e11d48`;
  ...
  const handleCopyLink = () => {
    if (!movie) return;
    const link = `https://vidsrc.wtf/api/1/movie/?id=${movie.id}&color=e11d48`;
