  ...
  if (isLoading) {
    return (
      <div className="flex flex-col justify-center items-center h-svh gap-6 bg-transparent">
        ...
        <p className="text-muted-foreground font-black tracking-[0.4em] uppercase text-[10px] animate-pulse">Syncing Cinematic Archive...</p>
      </div>
    );
  }
    ...
    <div className="relative min-h-svh bg-background pb-24">
            ...
            <div className="space-y-4 md:space-y-10 animate-in fade-in slide-in-from-bottom-10 duration-1000">
                <div className="flex flex-wrap items-center gap-2 md:gap-3">
                    <Badge className="bg-primary text-white font-black uppercase text-[8px] md:text-[10px] px-3 md:px-4 py-1 md:py-1.5 rounded-sm shadow-2xl shadow-primary/20 flex items-center gap-2">
                        <ShieldCheck className="size-2.5 md:size-3" /> Featured Cinema
                    </Badge>
                    <div className="flex items-center gap-1.5 md:gap-2 text-yellow-400 font-black text-[10px] md:text-sm bg-black/60 backdrop-blur-3xl px-3 md:px-4 py-1 md:py-1.5 rounded-full border border-white/10 shadow-2xl">
                        ...
                        {movie.vote_average.toFixed(1)} <span className="hidden sm:inline text-muted-foreground font-medium text-[10px] ml-1">Archive Grade</span>
                    </div>
      ...
      <div className="relative -mt-24 md:-mt-64 z-30 px-4 md:px-8 lg:px-12 max-w-[2000px] mx-auto">
          ...
          <div className="w-full lg:w-[450px] flex-shrink-0 space-y-8 md:space-y-12">
            ...
            <div className="grid grid-cols-1 gap-4">
                ...
                <div className="grid grid-cols-2 gap-3 md:gap-4">
                    <Button onClick={() => trailer && handlePlayTrailer(trailer.key)} variant="outline" className="rounded-xl md:rounded-2xl h-14 md:h-20 border-white/10 glass-panel text-[10px] md:text-sm font-black uppercase tracking-widest transition-all hover:scale-105 active:scale-95" disabled={!trailer}>
                        <Clapperboard className="mr-2 size-4 md:size-5" /> Trailer
                    </Button>
                    <Button onClick={handleSaveToggle} variant={isSaved ? "secondary" : "outline"} className="rounded-xl md:rounded-2xl h-14 md:h-20 border-white/10 glass-panel text-[10px] md:text-sm font-black uppercase tracking-widest transition-all hover:scale-105 active:scale-95" disabled={isSavedMovieLoading}>
                    ...
                    <div className="flex flex-wrap items-center gap-3 md:gap-4">
                        <div className="flex items-center gap-4 md:gap-8 bg-secondary/40 px-6 md:px-10 py-3.5 md:py-5 rounded-2xl md:rounded-[2.5rem] border border-white/5 backdrop-blur-3xl shadow-2xl">
                            <div className="flex items-center gap-2 md:gap-3">
                                ...
                                <span className="font-black uppercase tracking-widest text-[10px] md:text-xs">{movie.release_date ? new Date(movie.release_date).getFullYear() : 'TBA'}</span>
                            </div>
                            <Separator orientation="vertical" className="h-4 md:h-6 bg-white/10" />
                            <div className="flex items-center gap-2 md:gap-3">
                                <Clock className="size-4 md:size-5 text-primary" />
                                <span className="font-black uppercase tracking-widest text-[10px] md:text-xs">{Math.floor(movie.runtime / 60)}h {movie.runtime % 60}m</span>
                            </div>
                        </div>
                    ...
                    <div className="flex items-center justify-between border-b border-white/5 pb-6 md:pb-8">
                        <div className="flex items-center gap-3 md:gap-4">
                            ...
                            <div className="space-y-0.5 md:space-y-1">
                                <h2 className="font-headline text-2xl md:text-5xl font-black tracking-tighter uppercase mb-0">Featured <span className="text-primary">Architects</span></h2>
                                <p className="text-[8px] md:text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] md:tracking-[0.4em] opacity-60">The visionary ensemble behind this production.</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="space-y-10 md:space-y-12">
                        {directors.length > 0 && (
                            <div className="space-y-4 md:space-y-6">
                                <p className="text-[8px] md:text-[10px] font-black text-primary uppercase tracking-[0.4em] md:tracking-[0.5em] ml-1 md:ml-2">Production Lead</p>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6">
                                    {directors.slice(0, 2).map(director => (
                                        <Link href={`/person/${director.id}`} key={director.credit_id} className="flex items-center gap-4 md:gap-6 p-4 md:p-6 glass-panel rounded-2xl md:rounded-[2.5rem] hover:bg-white/10 transition-all border border-white/5 group shadow-2xl">
                                            <Avatar className="size-16 md:size-24 border-2 md:border-4 border-primary/40 group-hover:border-primary transition-all shadow-2xl">
                                                <AvatarImage src={getPosterUrl(director.profile_path)!} className="object-cover" />
                                                <AvatarFallback className="bg-secondary text-primary font-black text-xl md:text-2xl">{director.name.charAt(0)}</AvatarFallback>
                                            </Avatar>
                                            <div className="space-y-0.5 md:space-y-1 overflow-hidden">
                                                <p className="text-lg md:text-2xl font-black text-white group-hover:text-primary transition-colors leading-tight uppercase tracking-tighter truncate">{director.name}</p>
                                                <p className="text-[8px] md:text-[10px] text-muted-foreground uppercase font-black tracking-widest">Film Director</p>
                                            </div>
                                        </Link>
                                    ))}
                                </div>
                        ...
                        <div className="space-y-4 md:space-y-6">
                            <p className="text-[8px] md:text-[10px] font-black text-muted-foreground uppercase tracking-[0.4em] md:tracking-[0.5em] ml-1 md:ml-2">Primary Personnel</p>
                                ...
                                {movie.credits.cast.slice(0, 9).map(person => (
                                    <Link href={`/person/${person.id}`} key={person.credit_id} className="flex items-center gap-4 md:gap-5 p-4 md:p-5 glass-panel rounded-xl md:rounded-[2rem] hover:bg-white/10 transition-all border border-white/5 group shadow-xl">
                                        ...
                                        <div className="overflow-hidden space-y-0.5 md:space-y-1">
                                            ...
                                            <p className="text-[8px] md:text-[9px] text-muted-foreground uppercase font-bold truncate tracking-widest">{person.character}</p>
                                        </div>
                                    </Link>
                                ))}
                            </div>
        ...
        {similarMovies.length > 0 && (
            <div className="mt-24 md:mt-32 pt-16 md:pt-20 border-t border-white/5 space-y-8 md:space-y-12">
                <div className="flex items-center gap-3 md:gap-4">
                    <div className="p-2.5 md:p-3 bg-blue-400/10 rounded-xl md:rounded-2xl border border-blue-400/20 shadow-2xl">
                        <TrendingUp className="size-6 md:size-8 text-blue-400" />
                    </div>
                    <div className="space-y-0.5 md:space-y-1">
                        <h2 className="font-headline text-2xl md:text-5xl font-black tracking-tighter uppercase mb-0">Cinematic <span className="text-blue-400">Parity</span></h2>
                        <p className="text-[8px] md:text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] md:tracking-[0.4em] opacity-60">Related cinematic works from global transmission nodes.</p>
                    </div>
                </div>
