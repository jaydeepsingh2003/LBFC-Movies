...
import { addTvShowReview } from '@/firebase/firestore/tv-reviews';
import { TvShowRating } from './tv-show-rating';
...
export function TVReviewForm({ showId, onReviewSubmit }: ReviewFormProps) {
  const { user } = useUser();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      await addTvShowReview(firestore, showId, {
        userId: user.uid,
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        ...
        <TvShowRating showId={showId} />
      </div>
      <Textarea
        value={content}
        onChange={(e) => setContent(e.target.value)}
        placeholder="Share your thoughts on the TV show..."
