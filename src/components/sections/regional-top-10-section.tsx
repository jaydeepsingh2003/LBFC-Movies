            ...
            <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-8">
                <div className="flex items-center gap-4">
                    <div className="p-3 bg-red-500/10 rounded-2xl border border-red-500/20 shadow-2xl shadow-red-500/10">
                        <MapPin className="text-red-500 size-7 md:size-8" />
                    </div>
                    <div className="space-y-1">
                        <h2 className="font-headline text-3xl md:text-5xl font-black tracking-tighter uppercase text-white mb-0 leading-none">
                            Top 10 in <span className="text-red-500">{activeRegionLabel}</span>
                        </h2>
                        <p className="text-[10px] md:text-xs font-black text-muted-foreground uppercase tracking-[0.4em] opacity-60">Trending productions originating from this territory.</p>
                    </div>
                </div>

                <div className="flex items-center gap-4 p-1.5 glass-panel rounded-[1.5rem] border-white/5 overflow-x-auto no-scrollbar max-w-full xl:max-w-none shadow-2xl">
                    {REGIONS.map((region) => (
                        <button 
                            key={region.code} 
                            onClick={() => setActiveRegion(region.code)}
                            className={cn(
                                "h-12 px-6 rounded-xl text-[10px] md:text-xs font-black uppercase tracking-widest transition-all duration-500 whitespace-nowrap",
