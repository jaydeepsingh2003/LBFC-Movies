...
export default function KannadaTvSection() {
    ...
    useEffect(() => {
        const fetchShows = async () => {
            ...
            try {
                ...
                console.error("Failed to fetch Kannada TV shows:", error);
            } finally {
                setIsLoading(false);
            }
        }
    ...
    return (
         <div className="space-y-4">
            <h2 className="font-headline text-2xl font-bold tracking-tight text-foreground">Popular Kannada TV Shows</h2>
            {shows.length > 0 ? (
                <Carousel
                opts={{
                    align: "start",
