...
export default function TamilTvSection() {
    ...
    useEffect(() => {
        const fetchShows = async () => {
            ...
            try {
                ...
                console.error("Failed to fetch Tamil TV shows:", error);
            } finally {
                setIsLoading(false);
            }
        }
    ...
    return (
         <div className="space-y-4">
            <h2 className="font-headline text-2xl font-bold tracking-tight text-foreground">Popular Tamil TV Shows</h2>
            {shows.length > 0 ? (
                <Carousel
                opts={{
                    align: "start",
