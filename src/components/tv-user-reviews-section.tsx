...
import { TVReviewForm } from './tv-review-form';
import type { UserTvShowReview, ReviewData } from '@/firebase/firestore/tv-reviews';
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
export function TVUserReviewsSection({ showId }: { showId: number }) {
  ...
  const handleDelete = async (reviewId: string) => {
    if (!firestore) return;
    try {
      await deleteDoc(doc(firestore, `tv-shows/${showId}/reviews`, reviewId));
    ...
    return (
      <div className="space-y-4">
        {reviewsSnapshot?.docs.map((doc) => {
          const review = { id: doc.id, ...doc.data() } as UserTvShowReview & { id: string };
  ...
  return (
    <section className="space-y-6 pt-8">
      ...
      <TVReviewForm showId={showId} onReviewSubmit={() => {}} />
